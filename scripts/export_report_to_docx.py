...
def export_docx():
    ...
    time_block_rows = [
        ("00:00~04:00", "深夜偷排最高峰", "467.1 ppb", "7,489 次", "15.1%", "第一高發 (夜間巡邏跟監)"),
        ("04:00~08:00", "清晨逆溫壓制期", "438.9 ppb", "7,083 次", "14.3%", "第一優先 (黃金突擊窗口)"),
        ("20:00~24:00", "夜間排放蓄積期", "396.5 ppb", "6,745 次", "13.5%", "中優先 (周界駐點快篩)"),
        ("16:00~20:00", "傍晚風向轉換期", "305.0 ppb", "5,719 次", "11.4%", "常態監測"),
        ("12:00~16:00", "午後對流消散期", "166.7 ppb", "3,083 次", "6.1%", "一般巡查"),
        ("08:00~12:00", "晨間工廠開工期", "201.8 ppb", "2,910 次", "5.8%", "常態監測")
    ]
    ...
    if c4 and os.path.exists(c4):
        ...
        r = p_c4.add_run("分析圖表五：一日 6 大時段 VOC 濃度、超標累積次數與重度事件率對照圖")
        r.italic = True
        r.font.size = Pt(8.5)

    add_body_p(doc, "時段維度呈現【2.8 倍之極強信號】。深夜 00:00～04:00 平均濃度（467.1 ppb）與清晨 04:00～08:00（438.9 ppb）是午後 12:00～16:00（166.7 ppb）的 2.8 倍，重度超標發生率更從 6.1% 暴增至 15.1%，深夜與清晨超標次數合計高達 14,572 次（佔全天候超標總量的 44.1%）。因此，環保局科技稽查之核心出擊時段應絕對死守【清晨 04:30～06:30】，方能於落地濃度最高、事證最明確之時段精準查扣事證。")
