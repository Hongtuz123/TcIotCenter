...
- 特色：多執行緒平行下載 (ThreadPoolExecutor)、斷點續傳、錯誤重試、進度統計
"""
...
# 並發與重試參數
MAX_WORKERS = 16  # 平行下載執行緒數
RETRY_COUNT = 3   # 錯誤重試次數
...
def run(start_str=DEFAULT_START, end_str=DEFAULT_END, resume=True):
    ...
    for idx, d_str in enumerate(dates, 1):
        ...
        log.info(f"[{idx:4d}/{total_days}] ⬇️ 開始下載 {d_str} (1,401 站平行載入中)...")
        day_start = time.time()
    ...
    log.info("=" * 65)
    log.info(f"🎉 批次下載任務結束！")
    log.info(f"📊 統計結果: 總天數 {total_days} 天 | 成功 {success_cnt} | 略過 {skip_cnt} | 失敗 {fail_cnt}")
    log.info(f"⏱️ 總耗時: {total_elapsed/60:.1f} 分鐘")
