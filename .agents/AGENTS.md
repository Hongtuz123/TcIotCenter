...
## 📚 學理與公式回覆規則
當使用者詢問任何專案功能之「學理部分」時，必須遵循以下規則：
1. **提供當前執行公式**：列出專案中目前實際在使用的數學公式、演算法細節或物理簡化模型。
2. **提供每一段的參考文獻**：在每個理論或公式下方，附上對應的法規標準、環境部/美國 EPA 技術文件、學術論文或模型說明書（如 AERMOD Model Formulation Document、Briggs 擴散係數規範等）。
